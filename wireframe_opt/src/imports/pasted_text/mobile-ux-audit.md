...
Interactive Gamified Landing Page · Figma Make · Audited June 2026
34
out of 100
Overall Mobile UX Score
Strong visual concept, but nearly every foundational mobile UX requirement is violated. The page is essentially unreadable outdoors, conversion friction is high, and the form is buried 8+ scrolls deep.
...
Haptic-style micro-interaction on CTA tap. The gamified theme earns a subtle scale pulse (transform: scale(0.96)) on the START QUEST button press — zero performance cost, reinforces the game feel on mobile.
Progress bar for scroll depth. A thin colored progress line at the top (like a reading indicator) would reinforce the "quest" metaphor and give users a sense of how far they are through the page — reducing scroll abandonment.
Reduce motion query. Any entrance animations should respect prefers-reduced-motion: reduce. Users with vestibular conditions or older phones benefit; implementation is a 2-line CSS addition.
Native share button near hero. "SiviCamp 2026 · Frankfurt" is shareable. A small share icon next to the date line triggers the native iOS/Android share sheet — zero effort for the user, organic reach for the event.
Discord / Facebook quick-join button above footer. The social links (Facebook, Discord, LinkedIn) are currently text-only links in the bottom section. For the Vietnamese-diaspora audience, a prominent Discord CTA with icon would convert better than a registration form for undecided visitors.
Ideal mobile user flow (redesigned)
How the page should feel from hero → conversion on a real phone.
1
Hero (0–1s): Full-bleed dark bg, RAMMASTIC title loads instantly (no entrance delay). Date + location visible immediately below title. "REGISTRATIONS OPEN" badge in high-contrast green. Countdown timer below — readable at arm's length. ONE CTA: "Đăng ký ngay" — 52px tall, full-width, high-contrast purple fill with white text.
2
After first scroll (2–4s): 4 stat cards (100+ participants, 48h hacking, 3 workshops, 1st in Europe) — large numbers, legible labels. No decoration, just facts. Sticky bottom bar appears: "🎮 Đăng ký · Còn 100 chỗ" with a counter — mild urgency, persistent.
3
What is RAMmastic (4–8s): 2–3 sentence explainer. Below it, three section cards (SiviHack / SiviTa / SiviTour) as horizontally scrollable cards — each with an icon, 1-line description, and a "Xem thêm" tap to expand. Keeps the section short without losing content.
4
Eligibility (8–12s): Replace hover-flip cards with a simple checklist: 4 bullet criteria, each with a green check icon. No interaction needed — users self-qualify in 5 seconds.
5
Schedule (12–18s): Sticky 3-tab day selector (T5 / T6 / T7). Only the selected day's events shown. Collapsed by default with a "Xem tất cả" expand. Removes ~60% of scroll length from this section.
6
Registration CTA section (18–22s): Large "Sẵn sàng chưa?" heading. Event summary card (date, location, cost: Miễn phí). THEN the form — pre-filled where possible (name from browser autofill). Track selector uses large 48px radio buttons. Single "Đăng ký" submit — not "SUBMIT APPLICATION" in English on a Vietnamese-primary page.
7
Post-submit: Confirmation screen with quest metaphor ("Quest accepted!"), Discord join link, and calendar add button. Closes the loop in the gamified language of the design.